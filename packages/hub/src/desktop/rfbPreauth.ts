    ...
    private processServer(): void {
        switch (this.phase) {
            ...
            case 'security-types': {
                ...
                if (!canAuth) {
                    this.finish(false, types.includes(SECURITY_VNC_AUTH) ? 'vnc password not configured' : 'no supported security type')
                    return
                }
            ...
            case 'security-result': {
                ...
                this.deps.onToBrowser(encodeAuthFailure('vnc auth failed'))
                this.finish(false, 'vnc auth failed')
                return
            }
            default:
                return
