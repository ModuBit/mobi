 ...
 */
export type UsePinnedSessionsResult = ReturnType<typeof usePagedSessionList>

export function usePinnedSessions(
    activeSessionId?: string,
    /** 「置顶」为平级分区：有会话默认展开、空分区默认收起（用户 toggle 后以用户选择为准） */
    expandWithContent = true,
): UsePinnedSessionsResult {
    const api = useMobiApi()

    // 获取置顶会话 ID 列表（始终请求，避免折叠时无数据判断激活态）
    const query = useSessionIdsPages({
        queryKey: queryKeys.pinnedSessions,
        fetchPage: (cursor) =>
            api.sessions.pinnedSessions(cursor ?? undefined, PAGE_SIZE).then(res => res.data),
    })
