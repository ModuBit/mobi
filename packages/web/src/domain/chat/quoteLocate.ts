 ...
 *
 * 纯 DOM 动作、无 React 状态，由 UserBlocksView 的 env.onQuoteLocate 能力位接线
 * （对齐 onEditSketch 的可选模式：能力只在聊天列表接线处有意义，非聊天上下文退化为纯展示）。
 *
 * 高亮闪烁样式（{@link QUOTE_FLASH_CLASS} 的 keyframes 定义）由 React 树侧注入：
 * UserBlocksView 挂 emotion Global（本模块无渲染上下文，且 @emotion/react 不含 injectGlobal）。
 */

/** 高亮类名（UserBlocksView 的 Global 样式按此名定义闪烁动画；导出供测试断言） */
