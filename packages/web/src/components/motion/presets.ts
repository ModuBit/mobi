 ...
 *
 * bounce 统一为 0（无 overshoot，临界阻尼感）：真机体感 overshoot 容易动效过头
 * （产品决策 2026-08-21 去除），保留 spring 的速度继承与可打断性。
 *
...
export const spring = {
    /** 状态切换、开关、面板开合（默认档） */
    ui: { type: 'spring', duration: 0.35, bounce: 0 } satisfies Transition,
    /** 拖拽释放后的沉降（Drawer、甩动）——velocity 由调用方透传 */
    momentum: { type: 'spring', duration: 0.3, bounce: 0 } satisfies Transition,
    /** 大面积元素（Modal 级）位移 */
    gentle: { type: 'spring', duration: 0.5, bounce: 0 } satisfies Transition,
} as const
