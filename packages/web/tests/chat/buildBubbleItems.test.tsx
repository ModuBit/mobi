...
describe('buildChatBubbleItems', () => {
    ...
    describe('result 分隔线', () => {
        it('工具组后的首个 agent-text → 前插 result divider（过程结束、产出开始）', () => {
            const blocks = [
                createCompletedToolCall('tc1'),
                createCompletedToolCall('tc2'),
                createAgentText({ id: 'a1' }),
            ]
            const items = buildChatBubbleItems(blocks, defaultCtx, false, defaultOptions)
            expect(items.map(i => i.key)).toEqual(['group-tc1', 'result-divider-a1', 'a1'])
            expect(items[1].role).toBe('divider')
        })

        it('单个 tool-call（不成组）后的 agent-text → 同样插 result divider', () => {
            const blocks = [
                createCompletedToolCall('tc1'),
                createAgentText({ id: 'a1' }),
            ]
            const items = buildChatBubbleItems(blocks, defaultCtx, false, defaultOptions)
            expect(items.map(i => i.key)).toEqual(['tc1', 'result-divider-a1', 'a1'])
        })

        it('reasoning 后的 agent-text → 插 result divider（思考也是过程）', () => {
            const blocks = [
                {
                    kind: 'agent-reasoning' as const,
                    id: 'r1',
            ...
            const items = buildChatBubbleItems(blocks, defaultCtx, false, defaultOptions)
            expect(items.map(i => i.key)).toEqual(['r1', 'result-divider-a1', 'a1'])
        })

        it('连续 agent-text → 只在首个（紧跟过程块的）前插一次，后续正文不再插', () => {
            const blocks = [
                createCompletedToolCall('tc1'),
                createCompletedToolCall('tc2'),
                createAgentText({ id: 'a1' }),
                createAgentText({ id: 'a2' }),
            ]
            const items = buildChatBubbleItems(blocks, defaultCtx, false, defaultOptions)
            expect(items.map(i => i.key)).toEqual(['group-tc1', 'result-divider-a1', 'a1', 'a2'])
        })

        it('无过程块前置（用户消息/开头）的 agent-text → 不插', () => {
            const blocks = [
                createUserText({ id: 'u1' }),
                createAgentText({ id: 'a1' }),
            ]
        ...
        it('context-cleared 显式分区后的 agent-text → 不再叠加 result divider', () => {
            const blocks = [
                createCompletedToolCall('tc1'),
                createAgentEvent({ id: 'ev-ctx', event: { type: 'context-cleared' } as AgentEventBlock['event'] }),
                createAgentText({ id: 'a1' }),
            ]
            const items = buildChatBubbleItems(blocks, defaultCtx, false, defaultOptions)
            expect(items.map(i => i.key)).toEqual(['tc1', 'ev-ctx', 'a1'])
        })
    })
})
