...
function getFeatureFlag(platform: string, arch: string): string {
    ...
    return `MOBI_TARGET_${platformToken}_${arch.toUpperCase()}`;
}
...
function ensureEmbeddedAssetsManifest(workspaceRoot: string, includeWebAssets: boolean): void {
    ...
    if (includeWebAssets) {
        if (!existsSync(manifestPath)) {
            throw new Error(
                'Missing embedded web asset manifest. Run `bun run build:web` and `cd hub && bun run generate:embedded-web-assets`, or `bun run build:exe` from the repo root.'
            );
        }
        if (isStubEmbeddedAssets(manifestPath)) {
            throw new Error(
                'Embedded web asset manifest is a stub. Run `bun run build:web` and `cd hub && bun run generate:embedded-web-assets`, or `bun run build:exe` from the repo root.'
            );
        }
        return;
    }
...
async function main(): Promise<void> {
    ...
    const name = getArg(args, '--name') ?? 'mobi';
    const buildAll = args.includes('--all');
    const includeWebAssets = args.includes('--with-web-assets');

    if (args.includes('--target') && !target) {
        console.error('Usage: bun run scripts/build-executable.ts [--target <bun-platform[-arch[-variant]]>] [--outdir dist-exe] [--name mobi] [--with-web-assets]');
        console.error('   or: bun run scripts/build-executable.ts --all [--outdir dist-exe] [--name mobi] [--with-web-assets]');
        process.exit(1);
    }
