...
const RELEASE_URL = 'https://github.com/modu/mobi/releases';
...
function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = `@mobi/cli-${platformName}-${archName}`;

    try {
        const pkgPath = require.resolve(`${pkgName}/package.json`);
        const binName = platformName === 'win32' ? 'mobi.exe' : 'mobi';
...
function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    const platformPackage = `@mobi/cli-${platformName}-${archName}`;
    ...
    log(`  npm install -g @mobi/cli --registry=${OFFICIAL_NPM_REGISTRY}`);
    log('');
