...
main() {
    ...
    # 2. 创建临时数据目录
    e2e_log_info "创建临时数据目录: ${E2E_TMPDIR}"
    mkdir -p "${E2E_TMPDIR}"
    ...
    # 7. 启动 Runner（通过 CLI 加载 --profile e2e）
    e2e_log_section "启动 Runner"

    (
        cd "${MOBI_ROOT}" && \
        bun run packages/cli/src/index.ts --profile "${PROFILE_NAME}" runner start &>/tmp/mobi-e2e-runner.log
    )

    # 等待 Runner 状态文件出现并检查进程存活
    ...
    if [[ "${runner_ready}" == false ]]; then
        ...
        cat /tmp/mobi-e2e-runner.log 2>/dev/null || true
        cleanup
        exit 1
    fi
