...
- [跨会话投递验证](send-message-verify.md) — 造会话（curl 直调 spawn）/ 探针**必须点名工具**（否则收件方去用 CC 原生 SendMessage）/ meta+lifecycle+「恰好一行」断言 / 往返·扇出·建完即用·无幽灵的验法 / 富内容与**跨机器靠改 DB machineId**（改完必还原）/ Auto 也会卡审批的坑
