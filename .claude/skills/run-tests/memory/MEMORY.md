...
- [跨会话投递验证](send-message-verify.md) — 造一对会话（curl 直调 spawn）/ 探针**必须点名工具**（否则收件方去用 CC 原生 SendMessage）/ meta+lifecycle+「恰好一行」断言 / 富内容素材与 transcript 换算表 / **跨机器靠改 DB machineId**（改完必还原）/ 死目标与扇出
