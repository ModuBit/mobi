 ...
 * - MOBI_CLAUDE_PATH: Force a specific path to claude executable
 */
export function getDefaultClaudeCodePath(): string {
    // Allow explicit override via env var
    if (process.env.MOBI_CLAUDE_PATH) {
        logger.debug(`[Claude SDK] Using MOBI_CLAUDE_PATH: ${process.env.MOBI_CLAUDE_PATH}`)
    ...
    if (!globalPath) {
        throw new Error('Claude Code CLI not found on PATH. Install Claude Code or set MOBI_CLAUDE_PATH.')
    }
    return globalPath
}
