...
- 前端已消费真实数据：`normalizeAgent.ts` user-array → sidechain prompt → `tracer.ts` `promptToTaskId` 匹配挂链；`tool-card/index.tsx` `getAgentPrompt` 从 tool\_use.input.prompt 渲染卡片
...
**hooks 路线判死**：SDK 0.3.227 实装的 `Options.hooks` 只支持 command/prompt/agent/http/mcp\_tool 五种类型，**没有 JS 回调**——条目调研时「SDK hooks option JS 回调 ✅ 可行」的结论与实装版本不符。

**条目想要的其余部分早已存在**：Web UI（`teamAgentsStore` + `TeamAgentPanel` + `TeamAgentCard`）已创建在用（条目「待创建」过时）；Hub 的 team state 提取（TeamCreate/Agent/SendMessage/task\_started/task\_progress）与 all-done 自动清理（`applyTeamStateDelta` 尾部）均已实现。

**真正的缺口**（与 #44 同根因）：member `status: 'running'` 无出口翻终态。已由 tool\_result 消费方案解决，见 #44。
...
## 34. ~~`Auto` 模型下 claude 子进程静默挂死（无产出、无超时、无提示）~~ ✅ 已解决（2026-08-15 方案 1 实施：首响应静默提示）
...
**实施（2026-08-15，方案 1：首响应静默提示，纯前端）**：

- **缺口本质**：区分「在干活」vs「在干等」的信号是**静默时长**（距最后一条消息的时间），不是总耗时——正常长任务期间消息/工具事件持续到达，挂死则完全静默；而 vibing 动词照样轮换，造成「一直在干活」的假象
- `AgentLoadingBubble` 新增 `lastActivityAt` prop：静默 ≥ **120s**（实测 `/code-review high` 正常 ~85s 出首个事件，留余量）→ 文案切 `still waiting for response…` + 警示色；`awaiting_auth` 优先级更高（等待审批非模型无响应）；不传 prop 不启用（sidechain 卡片向后兼容）
- 数据链：`ChatContainer`（倒序找最后一条带 `positionAt` 的消息，含排队消息——用户刚发消息也是活动）→ `ChatComposer` → `StatusBar` → `AgentLoadingBubble`；hub/cli 零改动
- 方案 2（透出 claude 侧静默状态）不做：前端静默检测已覆盖可感知性，CLI runtime signal 上报收益边际
- 单测 4 用例（超阈值切换 / 未超不误报 / 未传向后兼容 / awaiting_auth 优先）

**涉及文件**：

- `packages/web/src/components/chat/AgentLoadingBubble.tsx` — 静默检测与文案/颜色切换
- `packages/web/src/components/chat/ChatContainer.tsx` / `ChatComposer.tsx` / `StatusBar.tsx` — lastActivityAt 透传链
...
- `ContextUsageThread` 的 `totalTokens` 取「最后一条 assistant 的 input+cache\_creation+cache\_read」，但多 assistant 消息/并发 agent 时会叠错基数；
- 百分比按 `totalTokens / maxTokens`，但 maxTokens 是 `result.modelUsage[model].contextWindow`，不同 model 窗口混算时比例失真；
- 未计入 tool\_result 输出、system prompt 等占用。
...
两个现象同一根因：**member 生命周期只有入口（Agent tool\_use 注册 `status: 'running'`）没有出口**——自动清理（`applyTeamStateDelta` 的 all-done 判定）早已存在，但 member 永远等不到 done，形同虚设。

**修复（零 hook、零 CLI 改动，hub 纯函数层）**：Agent 工具的 `tool_result` 本来就在消息流里，它到达即意味着该 teammate 已跑完：

- `TeamMemberSchema` 加 `toolUseIds`；`processTaskToolWithTeam` 存派发 tool\_use id
- 新增 `extractTeamMemberCompletionFromMessageContent`：扫 user 消息的 tool\_result，配对 `toolUseIds` → member + 对应 task（`agent:${name}`）翻 completed（is\_error 也算完成——失败的 teammate 也要退出）
- `sessionHandlers`（live）与 `backfillRuntimeStateFromMessages`（重放）两处接入

**现象二（清理不生效）随之消失**：重放按序处理历史（tool\_use 重建 → tool\_result 再标完成 → all-done 自动清空），清理后重放自然收敛为空，无需 tombstone。
...
**实现坑（DB 实证）**：消息 envelope 外层 `role` 恒为 `'agent'`（SDK 统一 envelope），真实消息类型看 `data.type`——解析 tool\_result 不能检查外层 role，对齐 `sync/tasks.ts` 的判定方式。
