...
| `mcp__mobi-core__change_title`（历史名 `mcp__mobi__change_title` / `mobi__change_title`，为老会话保留匹配） | 改标题工具 | 提取标题，生成 `title-changed` 事件（不渲染工具卡片本身）；CLI 侧 best-effort 回写 CC customTitle |
