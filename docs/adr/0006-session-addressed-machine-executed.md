...
4. **`save-file` 唯一例外，留 session socket**：写边界锚定会话 cwd 子树（runner 侧 saveFile 的 workingDirectory 是 runner 自己的 cwd，降级会写错位置）；且写操作天然发生在用户交互时刻，是将来「会话休眠/唤醒门」模型的写路径
